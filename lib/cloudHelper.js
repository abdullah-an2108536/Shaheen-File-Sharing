...
/**
 * Retrieves getOneDriveFolderId.
 */
async function getOneDriveFolderId(accessToken) {
...
/**
 * Retrieves the metadata.enc file from OneDrive (assumed to be in folder).
 */
export async function retrieveMetadataFromOneDrive(accessToken) {
  const folderId = await getOneDriveFolderId(accessToken);

  const url = `https://graph.microsoft.com/v1.0/me/drive/items/${folderId}:/content`;
...
export async function retrieveFileFromOneDrive(accessToken, fileName) {
  ...
  const url = `https://graph.microsoft.com/v1.0/me/drive/items/${folderId}:/${encodeURIComponent(
    fileName
  )}:/content`;
...
/**
 * List all metadata files in OneDrive
...
export async function getGoogleDriveStorageUsage(auth) {
  ...
  const used = Number(quota.usage) || 0; // ✅ TOTAL usage across Drive (not just app)
  const total = Number(quota.limit) || 15 * 1024 ** 3; // default 15GB if no limit
...
export async function getOneDriveStorageUsage(token) {
  let totalShaheenUsed = 0;
  const folderId = await getOneDriveFolderId(token);

  const folderUrl = `https://graph.microsoft.com/v1.0/me/drive/items/${folderId}/children`;
 

  // Fetch files under Shaheen folder
  try {
    ...
    if (!res.ok) {
      console.warn("⚠️ Failed to fetch /Shaheen folder:", res.statusText);
    } else {
      const data = await res.json();
      const files = data?.value || [];

      // Sum all file sizes
      totalShaheenUsed = files.reduce((sum, file) => sum + Number(file.size || 0), 0);

      // console.log("📁 Files in /Shaheen:", files.map(f => `${f.name} (${f.size} bytes)`));
      // console.log(`📦 Total used in /Shaheen: ${(totalShaheenUsed / 1024 ** 2).toFixed(2)} MB`);
    }
  } catch (err) {
    console.error("❌ Error accessing /Shaheen folder:", err.message);
  }

  const totalQuota = 5 * 1024 ** 3; // Assume 5GB OneDrive plan manually
  return {
    used: totalShaheenUsed,
    free: totalQuota - totalShaheenUsed,
    total: totalQuota
  };
}
